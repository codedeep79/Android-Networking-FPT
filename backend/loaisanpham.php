	...
	<?php 
		...
		function LayDanhSachKhuyenMai(){
			...
			$ngayhientai = date("Y/m/d");
			$truyvan = "SELECT * FROM khuyenmai km, loaisanpham lsp WHERE DATEDIFF(km.NGAYKETTHUC,km.NGAYBATDAU) >=0 AND km.MALOAISP = lsp.MALOAISP LIMIT 0 , 20";
			$ketqua = mysqli_query($conn,$truyvan);
		...
		function LayDanhSachSanPhamMoi(){
			...
			if($ketquacon){
				while ($dongtienich = mysqli_fetch_array($ketquacon)) {
					array_push($chuoijson, array("MASP"=>$dongtienich["MASP"],'GIATIEN'=> $dongtienich["GIA"],'TENSP' => $dongtienich["TENSP"],'HINHSANPHAM'=>"http://".$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT']."/shop24h".$dongtienich["ANHLON"]));
		...
		function LayLogoThuongHieuLon(){
			...
			if($ketquacon){
				while ($dongtienich = mysqli_fetch_array($ketquacon)) {
					array_push($chuoijson, array("MASP"=>$dongtienich["MATHUONGHIEU"],'TENSP' => $dongtienich["TENTHUONGHIEU"],'HINHSANPHAM'=>"http://".$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT']."/shop24h/".$dongtienich["HINHTHUONGHIEU"]));
		...
		function LayDanhSachSanPhamDanhMucTheoMaLoai($conn,$maloaisp,$chuoijson,$limit){
			...
			if($ketquacon){
				while ($dongtienich = mysqli_fetch_array($ketquacon)) {
					array_push($chuoijson, array("MASP"=>$dongtienich["MASP"],'TENSP' => $dongtienich["TENSP"],'GIATIEN'=>$dongtienich["GIA"],'HINHSANPHAM'=>"http://".$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT']."/shop24h".$dongtienich["ANHLON"],'HINHSANPHAMNHO'=>"http://".$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT']."/weblazada".$dongtienich["ANHNHO"]));
		...
		function LayDanhSachSanPhamTheoMaLoai($conn,$maloaisp,$chuoijson,$limit){
			...
			if($ketquacon){
				while ($dongtienich = mysqli_fetch_array($ketquacon)) {
					array_push($chuoijson, array("MASP"=>$dongtienich["MASP"],'TENSP' => $dongtienich["TENLOAISP"],'GIATIEN'=>$dongtienich["GIA"],'HINHSANPHAM'=>"http://".$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT']."/shop24h".$dongtienich["ANHLON"],'HINHSANPHAMNHO'=>"http://".$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT']."/weblazada".$dongtienich["ANHNHO"]));
				...
				while ($dong=mysqli_fetch_array($ketqua)) {
					...
					if($ketquacon){
						while ($dongphukiencon = mysqli_fetch_array($ketquacon)) {
							array_push($chuoijson, array("MASP"=>$dongphukiencon["MALOAISP"],'TENSP' => $dongphukiencon["TENLOAISP"],'GIATIEN'=>$dongphukiencon["GIA"],'HINHSANPHAM'=>"http://".$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT']."/shop24h".$dongphukiencon["ANHLON"]));
		...
		function LayDanhSachTopPhuKien(){
			...
			if($ketqua){
				while ($dong=mysqli_fetch_array($ketqua)) {
					...
					if($ketquacon){
						while ($dongphukiencon = mysqli_fetch_array($ketquacon)) {
							array_push($chuoijson, array("MASP"=>$dongphukiencon["MASP"],'TENSP' => $dongphukiencon["TENSP"],'GIATIEN'=>$dongphukiencon["GIA"],'HINHSANPHAM'=>"http://".$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT']."/shop24h".$dongphukiencon["ANHLON"]));
		...
		function LayDanhSachTopDienThoaiVaMayTinhBang(){
			...
			if($ketqua){
				while ($dong=mysqli_fetch_array($ketqua)) {
					...
					//cách 2
					array_push($chuoijson, array("MASP"=>$dong["MASP"],'TENSP' => $dong["TENSP"],'GIATIEN'=>$dong["GIA"],'HINHSANPHAM'=>"http://".$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT']."/shop24h".$dong["ANHLON"]));
					//end cách 2
				}
			...
			if($ketquamtb){
				while ($dongmtb=mysqli_fetch_array($ketquamtb)) {
					...
					//cách 2
					array_push($chuoijson, array("MASP"=>$dongmtb["MASP"],'TENSP' => $dongmtb["TENSP"],'GIATIEN'=>$dongmtb["GIA"],'HINHSANPHAM'=>"http://".$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT']."/shop24h".$dong["ANHLON"]));
					//end cách 2
				}
		...
		function LayDanhSachCacThuongHieuLon(){
			...
			if($ketqua){
				while ($dong=mysqli_fetch_array($ketqua)) {
					...
					//cách 2
					array_push($chuoijson, array("MASP"=>$dong["MATHUONGHIEU"],'TENSP' => $dong["TENTHUONGHIEU"],'HINHSANPHAM'=>"http://".$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT']."/shop24h".$dong["HINHLOAISPTH"]));
					//end cách 2
				}
		...
		function LayDanhSachMenu(){
			...
			//$truyvancha = "SELECT *  FROM loaisanpham";
			$ketqua = mysqli_query($conn,$truyvancha);
			$chuoijson = array();
			echo "{";
			echo "\"LOAISANPHAM\":";
